...
def EfficientNet(width_coefficient,
    ...
    import tensorflow as tf
    output = tf.identity(x, name='reduction_%s' % str(len(endpoints) + 1))
    endpoints['reduction_%s' % str(len(endpoints.keys()) + 1)] = output
    ...
    output = tf.identity(x, name='output_features')
    endpoints['features'] = output
...
class EfficientNetX(tf.keras.models.Model):
    ...
    def __init__(self, config, architecture):
        ...
        if architecture == 'efficientnet-b0':
            self.efficientnet, self.endpoints = EfficientNetB0(include_top=False, weights='imagenet')
        elif architecture == 'efficientnet-b1':
            self.efficientnet, self.endpoints = EfficientNetB1(include_top=False, weights='imagenet')
        elif architecture == 'efficientnet-b2':
            self.efficientnet, self.endpoints = EfficientNetB2(include_top=False, weights='imagenet')
        elif architecture == 'efficientnet-b3':
            self.efficientnet, self.endpoints = EfficientNetB3(include_top=False, weights='imagenet')
        elif architecture == 'efficientnet-b4':
            self.efficientnet, self.endpoints = EfficientNetB4(include_top=False, weights='imagenet')
        elif architecture == 'efficientnet-b5':
            self.efficientnet, self.endpoints = EfficientNetB5(include_top=False, weights='imagenet')
        elif architecture == 'efficientnet-b6':
            self.efficientnet, self.endpoints = EfficientNetB6(include_top=False, weights='imagenet')
        elif architecture == 'efficientnet-b7':
            self.efficientnet, self.endpoints = EfficientNetB7(include_top=False, weights='imagenet')
        else:
            self.efficientnet, self.endpoints = EfficientNetL2(include_top=False, weights='imagenet')

        self.C2 = self.endpoints['reduction_2']
        self.C3 = self.endpoints['reduction_3']
        self.C4 = self.endpoints['reduction_4']
        self.C5 = self.endpoints['features']
        self.efficientnet_base = tf.keras.models.Model(self.efficientnet.input, [self.C2, self.C3, self.C4, self.C5])
    ...
    def call(self, inputs, training):
        C2, C3, C4, C5 = self.efficientnet_base(inputs, training=training)
